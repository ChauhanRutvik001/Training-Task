$(document).ready(function () {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    // $("#" + registration_main_id).closest('tr').remove();
                    // table.fnDeleteRow( table.$('#' + registration_main_id)[0] );
                    // table.row($(this).closest("tr")).remove();
                    // $("#example tbody").find(registration_main_id).remove();
            ...
            success: function (response) {
                if (response && response.status && response.status === true && Array.isArray(response.response)) {
                    ...
                    var data = allData.find(function (record) {
                        // console.log('Expected ID:', registration_main_id);
                        // console.log('Record ID:', record.registration_main_id);
    ...
    function clearValidationErrors() {
        var form = $('.form');
        if (form.length > 0 && typeof form.validate === 'function' && typeof form.validate().resetForm === 'function') {
            form.validate().resetForm(); // Reset the validation form
    ...
    $('.form').submit(function (event) {
        ...
        $(this).validate({
            rules: {
                user_code: {
                    required: true,
                    minlength: 3, // Greater than 2
                    maxlength: 8  // Less than 6
                },
                first_name: {
                    required: true
                },
